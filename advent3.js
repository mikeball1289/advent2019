...
function parse(wire) {
    return wire.split(',').map(wp => {
        switch (wp[0]) {
            case 'U': return { d: UP, l: Number(wp.slice(1)) };
            case 'D': return { d: DOWN, l: Number(wp.slice(1)) };
            case 'L': return { d: LEFT, l: Number(wp.slice(1)) };
            case 'R': return { d: RIGHT, l: Number(wp.slice(1)) };
        }
    });
}
...
function traverse(wire, board, label) {
    let position = { x: 0, y: 0 };
    let collisions = [];
    let len = 0;
    for (const part of wire) {
        const step = stepInDirection(part.d);
        for (let i = 0; i < part.l; i ++) {
            position.x += step.x;
            position.y += step.y;
            const cell = board[`${position.x}|${position.y}`];
            if (cell == null) {
                board[`${position.x}|${position.y}`] = `${label}${len + i}`;
            } else if (cell[0] != label) {
                collisions.push({ ...position, dist: Number(cell.slice(1)) + len + i });
            }
        }
        len += part.l;
    }
...
let board = {};
traverse(parse(wire1), board, 'a');
const collisions = traverse(parse(wire2), board, 'b');
console.log(Math.min(...collisions.map(p => p.dist)) + 2); // +2 because they include the origin in the wire distance but I don't